...
% In our previous work \cite{ecai/BogaertsGCG20}, we observed that, for \textit{difficult} puzzles, some explanation steps were still quite hard to understand. An example is depicted in Figure \ref{fig:pasta_diff}.
...
% While equally straightforward for a computer, understanding such an indirect proof, using contradiction, in a single step is notably harder for a person than the more common forward reasoning explanations our tool generates. Hence, we expanded our tool to -- on demand -- generate a \textit{nested} explanation of this contradiction reasoning.
% If a user indicates that they do not understand why a certain fact is derived, (in this case, the fact Farfalle does not cost \$8), we initiate a new (nested) explanation sequence that starts from the negation of that fact and derives a contradiction. 
% As such, this gives the user the option to zoom in into the difficult explanation steps, especially those where multiple (implicit or explicit) constraints are combined. 

In section \ref{sec:nested-explanation}, we have introduced the notion of nested explanations to refine those individual explanation steps in the sequence that are still too hard to understand. Subsequent algorithm \ref{alg:nested-main} is the extension of the greedy algorithm presented in algorithm \ref{alg:main} to support nested-explanations.
...
Fundamentally, for a given explanation step $(E, S, N)$, algorithm \ref{alg:nested-explanation} exploits only the previously derived facts $E$, the constraints and the newly derived facts used in the explanation step. 
