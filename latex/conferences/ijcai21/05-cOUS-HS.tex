...
\begin{example}
	\emilio{ Consider the following cnf $\formula$ formula over variables $x_1, x_2, x_3$ with model $\{x_1, \lnot x_2, x_3 \}$:
	...
	Let $\setstohit$ be the collection of hitting sets. We can help the hitting set solver by bootstrapping it with $\setstohit  =\{ \{ c_6, c_7\} \}$, to ensure at least one of the two is taken. We also add the `\textit{exactlyone}' constraint. Table \ref{tab:explanation-steps-expanded} sketches the steps required to find an explanation for \formula.}
\end{example}
...
\begin{proof}
	\emilio{
	The complexity of extracting a cardinality-minimal MUS (SMUS) for an unsatisfiable cnf $\formula$ is of the second level of polynomial hierarchy FP$^{\sum^{P}_2}$ \cite{ignatiev2015smallest}. If uniform weights are applied to \comus then it is reduced to computing an SMUS.}
