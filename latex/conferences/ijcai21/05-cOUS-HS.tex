...
Rephrased in these terms, the task of the procedure \onestep is to compute an OCUS of the formula $\formula := \formulac\land I\land \overline{\Iend\setminus I}$ with $p$ the predicate that holds for subsets  that contain exactly one literal of $\overline{\Iend\setminus I}$, see \cref{alg:oneStepOCUS}. 
...
Similar to OCUS the algorithm of \citet{ignatiev2015smallest} contains a \grow. However, the grow is optional: ``\emph{one can avoid the computation of MCS $\m{C}$ and use any correction subset $\m{C}'$ s.t. $\m{C} \subseteq \m{C}'$... Therefore, the grow procedure can be skipped}''.
In our case, OCUS requires to account for the cost, as well as the meta-level constraints.
...
Perhaps surprisingly, correctness of the proposed algorithm does \emph{not} depend on monotonicity properties of $f$ nor $p$. In principle, any (computable) cost function and condition on the unsatisfiable subsets can be used. In practice however, one is bound by limitations of the chosen hitting set solver.
 ...
 We provide an example of an \comus-call: 
\begin{example}
	Given the following CNF formula $\formulac$ over variables $x_1, x_2, x_3$:
		\[ c_1 = \lnot x_1 \vee \lnot x_2 \vee x_3 \text{\hspace*{20pt}} c_2 = \lnot x_1 \vee  x_2 \vee x_3\] \[   c_3 = \lnot x_3 \vee \lnot x_2 \]
	 If the current interpretation $I= \{ x_1\}$ and the final interpretation $\Iend = \{x_1, \lnot x_2,  x_3\}$, then $\overline{\Iend\setminus I} =  \{x_2, \lnot x_3\}$. We associate a clause for every literal $\m{I}$ and in  $\overline{\Iend\setminus I}$: 
	 \[ c_4 = \{x_1\}\text{\hspace*{20pt}} c_5=\{x_2\} \text{\hspace*{20pt}}c_6 = \{\lnot x_3\}\]
	 We define $p\triangleq$ \textit{exactlyone($c_5$, $c_6$)} and $f = \sum w_i$ the weighted sum over the assigned clause weights $w_1 = 20, w_2=20, w_3=20, w_4=1, w_5=1, w_6=1$.
	 
	 Given \formulac, $f$, $\m{I}$ and $\Iend$ we have an OCUS-call with
	 \[\formula= \formulac\land I \land \overline{(\Iend\setminus I)} = \{ c1 \wedge c_2 \wedge c_3\wedge c_4\wedge c_5\wedge c_6\}\]	
	 
	Let $\setstohit$ be the collection of hitting sets. We can help the hitting set solver by bootstrapping it with $\setstohit  =\{ \{ c_5, c_6\} \}$, since we know at least one of the two clauses has to be taken. We also add the `\textit{exactlyone}' constraint to the hitting set solver. Table \ref{tab:explanation-steps-expanded} sketches the steps required to find an OCUS for \formula given $f$ and $p$.\end{example}
\begin{table}[!h]
	\centering
	\begin{adjustbox}{max width=\columnwidth}
			\begin{tabular}{|r||c|c|c|c|}
				...
				\hline
				\rule{0pt}{2ex} 0 & - &  -&-  & \rule{0pt}{2ex} $\{ \{ c_5, c_6\} \}$  \\
				\hline
				1 &\rule{0pt}{2ex} $\{ c_5 \}$ & True & $\{c_1, c_2, c_3, c_5, c_6\}$    & $\{ \{ c_5, c_6\}, \{c_4\}\}$\rule{0pt}{2ex}   \\
				\hline		
				\multirow{2}*{2} &\rule{0pt}{2ex}\multirow{2}*{ $\{ c_5, c_4\}$}  & \multirow{2}*{True} & \multirow{2}*{$\{ c_1, c_2, c_4, c_5\}$}   & $\{ \{ c_5, c_6\}, \{c_4\}$  \\  
				& &  &    & $\{c_3, c_6\}\}$\rule{0pt}{2ex} \\\hline
				\multirow{2}*{3}&\rule{0pt}{2ex} \multirow{2}*{$\{ c_5,c_4, c_3 \}$}  & \multirow{2}*{True} &  \multirow{2}*{$\{c_2, c_3, c_4, c_5,c_6 \}$}  & $\{ \{ c_5, c_6\}, \{c_4\}$  \\  
				&&  &    & $\{c_3, c_6\}, \{c_1\}\}$\rule{0pt}{2ex} \\\hline
				4& \rule{0pt}{2ex} $\{ c_1, c_3, c_4, c_5 \}$ & False & \multicolumn{2}{c|}{$\{ c_1, c_3, c_4, c_5 \}$ is an OCUS of $\formula$.} \\
				\hline
			\end{tabular}
	\end{adjustbox}
...
%<<<<<<< HEA
Line 0 in table \ref{tab:explanation-steps-expanded} is the initialization phase, i.e bootstrapping the \setstohit. The next lines represent the successive hitting sets $\m{S}$ computed by the hitting set solver. The call to \grow exemplifies a call to \maxsat solver with hard clauses (or infinite weights) on the hitting set clauses and uniform weights on the remaining clauses of $\formula$. We further detail in section~\ref{sec:experiments} which \grow-configuration works best. Finally, note how at line 2, the hitting set computed is not $\{ c_5, c_6 \}$ of cost 2 due to the `\textit{exactlyone}'-constraint.
