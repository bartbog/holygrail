...
\begin{algorithm}[ht]
  ...
  \For{$(\m{S},M)\in\satsets$ \label{line:for-inc-seed}}{
    $\m{S}_\formula \gets \m{S}\cap \formula$\;
    \If{$\lnot \exists \m{S}'\in\satsets_\formula: \m{S}_\formula \subseteq\m{S}'$}{
        $\satsets_\formula\gets\satsets_\formula\cup\{\m{S}_\formula\}$\;
        $\setstohit \gets \setstohit\cup \{\formula \setminus \grow( \m{S}_\formula, \formula,M)\}$
        }
        }
  \While{true}{
...
%     (sat?, $\kappa$) $\gets$ \texttt{SatSolver}($hs$)\;
    ...
    \;
  }  \caption{$\omusinc(\formula,f,\setstohit=\emptyset)$ }
  \label{alg:omus-inc}
\end{algorithm}
% 
