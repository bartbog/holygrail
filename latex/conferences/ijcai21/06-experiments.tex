...
To our surprise, we noticed that \emilio{both in the constrained and non-constrained OUS setting, incrementality had no noticeable effect. Even more so, the naive restart of \textit{OUS} and \textit{OCUS} provide improvements on the more difficult puzzles. One potential explanation is that when keeping the MIP solver warm, such as Gurobi, similar branch-and-bound based solver will start in a search space close to the previous solution. However there is no immediate guarantee that the new best explanation is closely related to the previous one.}
