...
\emilio{As a final experiment, we investigate what the typical runtime is to explain one individual explain step for the top 3 configurations: \emph{\comus}, \emph{\comus+Incr.~HS} and \emph{OUSb+Lit.~Incr.~HS}. For both \emph{\comus} configurations, we see that $50\%$ of the explanations take less 1 second to be generated. After 5 seconds, approximately $87\%$ of the explanations have been generated. In contrast, for \emph{OUSb+Lit.~Incr.~HS} it takes less than 2 second to generate $87\%$ of the explanations except for 1 explanation step which takes more than 1000 seconds. This expensive explanation step explains the jump on Figure~\ref{fig:incrementality_constraindness} not only for \emph{OUSb+Lit.~Incr.~HS} but for all \emph{OUSb} configurations.}
