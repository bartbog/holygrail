...
\emilio{The result of algorithm \ref{alg:comus} represents a subset of the clauses of F. Consider the following cnf $\formula$ formula over variables $x_1, x_2, x_3$ with model $\{x_1, \lnot x_2, x_3 \}$.:
	...
	Let $\setstohit =\{ \{ c_6, c_7\} \}$ be the collection of hitting sets bootstrapped with the `\textit{exactlyone}' constraint. Table \ref{tab:explanation-steps-expanded} sketches the steps required to find an explanation for \formula.
}
