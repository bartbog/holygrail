...
\begin{algorithm}[H]
  ...
  \Input{$f$, \textit{a cost function $f : 2^{{\cal C}} \rightarrow  \mathbb{N} $ over CNF ${\cal C}$}}
  \Input{$I$, \textit{a partial interpretation over $U$ }}
  \Output{$E$, \textit{a sequence of explanation steps as tuples $I_{expl} \implies N_{expl}$}}
  \DontPrintSemicolon
...
\begin{algorithm}[H]
  ...
  \Input{$\uservars$, \textit{a set of variables.} }
...
\begin{algorithm}[H]
  \Input{${\cal C}$, \textit{a CNF}.}
  \Input{$f$, \textit{a cost function $f : 2^{{\cal C}} \rightarrow  \mathbb{N} $ over CNF ${\cal C}$}.}
  \Input{$p$, \textit{a predicate $p: 2^{{\cal G}} \rightarrow \{ t, f\}$}.}
