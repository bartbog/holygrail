...
where $X$ is a set of MIP decision variables, \bart{was:\sout{one for every literal that appears in $f$, $p$ or \setstohit} if im not mistaken should be:}one for every clause in $\formula$. In our case, $p$ is expressed as $\sum_{x \in \overline{\Iend\setminus I}} x = 1$. 
