...
In that setting, for $T$, we take $\allconstraints$ \emilio{$T_P ?$} for some puzzle $P$, as described in Section \ref{sec:prelims}. 
...
\begin{itemize}
 \item In the generation of the nested explanation, as well is in the high-level sequence, we used the greedy algorithm from the previous section. 
 ...
 The very first propagation in the nested explanation of this kind. \todo{verb missing ?}
