...
For each node $(E$, $\m{S}$, $n)$ of the graph, we have a structure that contains four components:
\begin{itemize}
  \item $n$.\texttt{state} a partial interpretation
  \item $n$.\texttt{Parent} parent explanation
  \item $n$.\texttt{Action} E (\textit{facts used}), S (\textit{Constraint used})
  \item $n$.\texttt{Path-Cost} the cost denoted by f(E, S, n) + \# elements until goal
\end{itemize}
...
\begin{algorithm}
  ...
  \Begin{
    \tcp{State with current interpretation}
    $\m{I} \gets \m{I}_0$ \;
    \tcp{Goal state}
    $\m{I}_{end} \gets$ \texttt{propagate($\m{I}$, $\m{T}$ )} \;
    \tcp{nodes to expand}
    $frontier \gets$ \textit{Priority queue ordered by Path-Cost + cost to goal, with $\m{I}$ as only element} \;
    \tcp{explanation sequence}
    $Seq \gets$ \textit{empty set} \;
    % $explored \gets$ \textit{empty set} \;
    \While{  true ? }{
      $(E_n$, $\m{S}_n$, $n) \gets$ \texttt{POP($frontier$)} \;
      \emilio{$nested \gets$ \texttt{nested-explanation($E_n$, $\m{S}_n$, $n$)}}\;  
      \emilio{append $((E_n$, $\m{S}_n$, $n), nested)$ to $Seq$} \;
      $\m{I} \gets$ $\m{I} \ \cup n$  \;
      \If{$\m{I} == \m{I}_{end}$}{
        \Return $Seq$
      }
      \tcp{for every subset of cosntraints}
      \For{$\m{S} \subseteq \m{T}$ ordered ascending by g($\m{S}$)}{
        $\m{J} \gets$ \texttt{propagate($\m{I}$, $\m{S}$)} \;
        \tcp{For every newly derivable fact}
        \For{$n_j \in \m{J} \setminus \m{I}$}{
          \emilio{$\neg n_j$ has to be in initialisation of H, as well as S} \;
          $X \gets$ \texttt{OMUS($\neg n_j \wedge \m{I} \wedge \m{S}$)} \;
          $E \gets$ $\m{I} \cap X$\;
          % $S \gets$ $\m{T} \cap X$\;
          $N \gets$ \texttt{propagate($E \wedge \m{S}$)} \;
          \For{$n' \in \m{N} $}{
            \uIf{$n' \notin frontier$}{
              add ($E$, $\m{S}$, $n'$) to frontier \;
            }
            \uElseIf{$n' \in  frontier$ with higher \texttt{Path-Cost}}{
              replace previous explanation with ($E$, $\m{S}$, $n'$) in Frontier \;
            }
          }
        }
      }
