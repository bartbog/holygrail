...
\title{Efficient Explanations of CSPs with Constrained Optimal Unsatisfiable Subsets}
...
\begin{abstract}
    ...
    explaining the solution-finding process in a human-understandable way. 
    The resulting explanation there is a 
    sequence of `simple' inference steps, where the difficulty of an inference step depends on the number and types of constraints used.
    While the algorithms developed there generate good explanations, they suffer from performance problems, and cannot guarantee that the result is optimal with respect to the defined difficulty. 
    To overcome these limitations, we define the problem of \emph{constrained optimal unsatisfiable subset (cOUS) generation}. 
    We develop a generic, incremental, algorithm for computing cOUSs, inspired by the well-known hitting set duality that lies at the basis of many \maxsat algorithms,    
    as well as optimizations of the algorithm specifically for explanation generation.  
    Our experimental results show that this results in qualitative improvements of the resulting explanation, as well as in an order of magnitude speed-up in the time needed to generate the explanation sequence. 
...
\section{A Basic cOUS Algorithm} 
% \todo{basic algo}
\input{05-cOUS-HS}

% \todo{Variants of the algorithm}

\section{Efficient cOUS Computation for Explanations}
