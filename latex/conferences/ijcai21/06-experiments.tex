...
Figure \ref{fig:grow_strategies} depicts the average (over all the puzzles) cumulative explanation time to derive a number of literals. Note that most puzzles only contain 150 literals, and hence the 'jump' seen is due to averaging over fewer puzzles. When a method times out for a puzzle at one step, a runtime value of 7200 is used in computing the averages for all future steps.
...
When comparing the configurations, we see that the plain \emph{MUS}-based implementation is faster than the \emph{O(C)US} implementations, as it solves a simpler problem. %cannot keep up with , as is to be expected since a harder problem is tackled.
Replacing the MUS calls by plain OUS calls leads to a much larger computation cost. The generic SS caching technique adds additional overhead. 
