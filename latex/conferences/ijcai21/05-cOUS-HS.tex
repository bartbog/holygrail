...
\emilio{Similar to \citet{ignatiev2015smallest}, the result of algorithm \ref{alg:comus} represents a subset of the clauses of F if applied on a toy example with imaginary weights.}
\todo{Add an example similar to SMUS ?}
\emilio{Consider the cnf $\formula$ formula over variables $x_1, x_2, x_3$ with final interpretation $\{x_1, \lnot x_2, x_3 \}$, $p\triangleq$ \textit{exactlyoneof($c_6$, $c_7$)} and $f = \sum w_i$ the sum over clause weights $w_1 = 20, w_2=20, w_3=20, w_4=20, w_5=1, w_6=1, w_7=1$:
...
%	, by adding $w_i - 1$ relaxation variables $(r_{i,1} \wedge .. \wedge r_{i,n-1}) \leftrightarrow c_i$, the problem of OUS is reduced to the problem of SMUS.
	...
	\noindent\textit{Constrainedness.} \todo{Add proof for Constrainedness}
\end{proof}
