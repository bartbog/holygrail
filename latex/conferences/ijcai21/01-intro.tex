...
Building on old ideas to explain propagations performed by constraint solvers  \cite{sqalli1996inference,freuder2001explanation}, \citet{ecai/BogaertsGCG20} recently introduced a 
...
Explanations in that work are sequences of simple inference steps, involving as few constraints as possible. 
In order to generate these sequences, \citeauthor{ecai/BogaertsGCG20} rely heavily on calls for  \emph{Minimal Unsatisfiable Subsets} (MUS) of a derived program, exploiting a one-to-one correspondence between so-called \emph{non-redundant explanations} and MUSs. 
The explanation steps are heuristically optimized with respect to a given cost function that should approximate human-understandability. 
...
\item Finally, building on the observation that many OCUS calls will have similar input,  we develop techniques for \textbf{information re-use} that allow speeding up consecutive OCUS calls.
