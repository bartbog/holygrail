...
\begin{algorithm}[t]
    ...
    % \vspace*{0.01cm}
    \DontPrintSemicolon
    \caption{$\call{ExplainCSP}(\formulac, f, I, U)$ \tias{OPTIONAL}}
    \label{alg:explainCSP}
    %$\sat \gets \initsat({\cal C}$) \;
        % \tcp{Hyp: f}
    $I_{end} \gets \optprop(\formulac \wedge I) \cap U$ \;
    ...
    \While{$(I \cap U) \neq I_{end}$}{
        ...
        $N \gets \optprop(X \cap I) \cap (\Iend \setminus I)$\;
...
The experiments of \citet{ecai/BogaertsGCG20} show that generating a full explanation sequence can easily take hours, and hence that algorithmic improvements are needed to make it more practical. 
