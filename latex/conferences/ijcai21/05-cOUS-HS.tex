...
\begin{definition}
   Let $\formula$ be a formula, $f:2^{\formula} \to \nat$ a cost function and  $p$ a predicate $p: 2^{\formula}\to \{true,false\}$. We call %a set $U\subseteq \formulag$ a \emph{$p$-constrained $f$-OUS} of \formulag ($(p,f)$-OUS) \tias{what with the OCUS name here?} \bart{I propsoe to say. We call 
    $\m{S} \subseteq \formula$ an OCUS of \formula (with respect to $f$ and $p$) if \begin{itemize}                                      
      ...
      \item $p(\m{S})$ is true
      \item all other unsatisfiable $\m{S}'\subseteq \formula$ for which $p(\m{S}')$ is true also satisfy $f(\m{S}')\geq f(\m{S})$.
    \end{itemize}
\end{definition}
...
The result of algorithm \ref{alg:comus} represents a subset of the clauses of F. We provide an example of an \comus-call: 
\begin{example}
	\emilio{
	Given the following CNF formula $\formulac$ over variables $x_1, x_2, x_3$:
	 ...
	 if we already know $I= \{ x_1\}$ from $\Iend = \{x_1, \lnot x_2,  x_3\}$, then $\overline{\Iend\setminus I} =  \{x_2, \lnot x_3\}$. We associate a clause for every literal $\m{I}$ and in  $\overline{\Iend\setminus I}$: 
	...
	Let $\setstohit$ be the collection of hitting sets. We can help the hitting set solver by bootstrapping it with $\setstohit  =\{ \{ c_5, c_6\} \}$, since we know at least one of the two clauses has to be taken. We also add the `\textit{exactlyone}' constraint to the hitting set solver. Table \ref{tab:explanation-steps-expanded} sketches the steps required to find an OCUS for \formula given $f$ and $p$.}
\begin{table}[!h]
	\centering
