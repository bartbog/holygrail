...
\todo{TIAS LOOKS AT IT and places some of this text in 5?1. } 
...
The second direction is to provide a \emph{more detailed} explanation of some of the more harder steps. 
...
The second direction on the other hand is interesting for several reasons. First of all, when implementing this, we noticed that some of the generated steps are still too complicated to be understood easily and thus really require being explained in more detailed. Secondly, breaking them up in smaller steps is an interesting challenge. Indeed, since we start from the idea that steps should be as simple as possible, it should not be possible to break them up further. 
To achieve this, we took inspiration from methods humans use when solving puzzles, and mathematicians use when proving theorems. 
That is, for explaining a single step in more detail we work by reasoning by contradiction: after assuming the negation of the derived fact, we present a sequence of small explanation steps that leads, starting from this negated assumption, to an inconsistency. 
As such, in the end this provides a mechanism that allows \emph{zooming in} onto the difficult explanation step.
