...
In the finite domain setting, the vocabulary, the interpretation of the types (the domains) and the constants are be fixed.\tias{Bart, please check that this sentence is still correct, e.g. it is fixed because FD}
This justifies the following definition. 
\begin{definition}
 A \emph{(partial) interpretation} is a set of literals, i.e., expressions of the form $P(\ddd)$ or $\lnot P(\ddd)$ where $P$ is a predicate symbol\tias{inconsistent terminology: a type symbol, a typed constant symbol or a relation symbol?} and $\ddd$ is a tuple of domain elements\tias{is?} of the types prescribed by the signature of $P$. A partial interpretation is \emph{consistent} if it does not contain both an atom and its negation, it is called a \emph{full} interpretation if it either contains $P(\ddd)$ or $\lnot P(\ddd)$ for each well-typed atom $P(\ddd)$. 
\end{definition}
...
Viewing (partial) interpretations as sets of literals allows us to also use them interchangeably as a theory\tias{a theory is a set of sentences reads the above, so this is weird... instead, 'use them as part of a theory when solving'? For me there is only one theory, the CSP, but for you there can be multiple, a reader like me may not see that or be confused}; we will make use of this in our algorithms.
