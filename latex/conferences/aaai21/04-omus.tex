...
\begin{definition}
  A MUS $M$ of \formula such that no MUS $M'$ exists with $f(M')<f(M)$  is called an \emph{$f$-Optimal Unsatisfiable Subset} (OUS) of \formula.
\end{definition}
...
If it is not, a procedure \grow is used to extend it to a set $\formula''$ with $\formula'\subseteq \formula''\subseteq\formula$ such that $\formula''$ is still satisfiable.
