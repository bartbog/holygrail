...
\revision{With these four observations in mind, we devised $f$ and $g$ as follows (where $nc(S)$ denotes the number of \emph{clues} (not: implicit constraints) in $S$, $M$ and $N$ are explanation parameters): \label{sec:cost}
...
&g(S) = basecost(S) = \left\{\begin{array}{ll}
                              ...
                              \end{array}\right.
                              \end{align*}
The parameters $M$ is taken here to be larger than any reasonable explanation size $|E| + N \cdot|S|$ and $N$ to be relatively small in comparison, but slightly larger than a few facts $|E|$. In our experiments, we use the combination $M=100$ and $N=5$ which provided good explanation sequences for the tested puzzles.}
