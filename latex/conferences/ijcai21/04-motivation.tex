Our work is motivated by the problem of explaining satisfaction problems through a sequence of simple explanation steps~\cite{ecai/BogaertsGCG20}. This can be used to teach people problem-solving skills, to compare the difficulty of related satisfaction problems (through the number and complexity of steps needed), and in human-computer solving assistants.
...
\emilio{ a  bit of a weird transition}
This algorithm algorithm starts from a formula $\formulac$ (in the application coming from a high level CSP), a cost function $f$ quantifying the difficulty of an explanation step, a partial interpretation $I$ and a set $U$ of atoms whose literals need explaining (e.g., not containing auxiliary variables). \emilio{not sure why we talk about U and not later in the text. U is especially useful with the call to the optimal propagate}
...
\begin{inparaenum}
 ...
 \item Finally, the algorithm that computes an entire explanation sequence makes use of repeated calls to \onestep and hence will solve many very similar problems. This raises the issue of \textit{\textbf{incrementality}: can we re-use data structures to achieve speed-ups in later calls}? \emilio{can we re-use computed knowledge to achieve speed-ups in later calls?}
\end{inparaenum}
