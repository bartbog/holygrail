...
In our practical implementation we use a cost function $f$ as well as a condition $p$ that can easily be encoded as linear constraints, and use the MIP solver \todo{WHICH ONE?} to compute such hitting sets. 
The choice of the underlying hitting set solver will thus determine which types of cost function and constraints are possible. 
...
%     (sat?, $\kappa$) $\gets$ \texttt{SatSolver}($hs$)\;
    ...
    $\F'' \gets  \grow(\F',\F) $ \label{line:grow}\;
...
Similar to algorithms for maximum satisfiability \cite{} and smallest MUS computation \cite{}, our algorithm builds on the hitting set duality of \cref{prop:MCS-MUS-hittingset}. 
Taking abstraction of \cref{line:grow} for a moment, 
the algorithm alternate calls to a hitting set solver with calls to a \sat oracle on a subset $\formula'$ of $\formula$. 
