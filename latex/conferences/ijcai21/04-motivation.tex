...
The algorithm used by \citeauthor{ecai/BogaertsGCG20} to find the next explanation step is shown in \cref{alg:oneStep}. 
It takes as input the formula \formulac, a cost function $f$ quantifying quality of explanations, an interpretation $I$ containing all already derived literals and a second interpretation $\Iend$. 
...
The experiments of \citet{ecai/BogaertsGCG20} show that even for simple puzzles, generating a full explanation sequence already takes several hours, and hence that algorithmic improvements are needed. 
We see three main points of improvement, all of which will be tackled by our generic cOUS algorithms presented in the next section. 
\begin{inparaenum}
 ...
 \item Second, this algorithm uses \call{MUS} calls for every literal to explain separately. The goal of all of these calls is to find a single unsatisfiable subset of $\formulac \land I \land \overline{(\Iend\setminus I)}$ that contains exactly one literal from $\overline{\Iend})$. This begs the questions whether it is possible \textbf{to compute (optimal) unsatisfiable subsets subject to constraints} (in our case, the constraint is on the number of literals from $\overline{\Iend}$ to include. 
...
\begin{definition}
\bart{I propsoe to say. We call U an OCUS of G (w.r.t.\ $f$ and $p$)}
...
Rephrased in this terms, the task of the procedure \onestep is to compute a $(p,f)$-OUS of the formula $\formulag := \formulac\land I\land \overline{\Iend\setminus I}$ where $p$ is the predicate that holds exactly for those subsets of $\formulag$ that contain exactly one literal of $\overline{\Iend}$. 
