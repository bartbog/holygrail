...
While our OCUS algorithm in~\ref{alg:comus} is generic and can also be used to find OUSs, its constrainedness property is key to decreasing the complexity of explanation sequence generation. We now discuss the optimizations to the OCUS algorithm that are specific to explanation sequence generation, though they can also be used when other forms of domain knowledge are present. 
