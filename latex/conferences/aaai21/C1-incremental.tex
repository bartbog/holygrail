...
These correction subsets themselves are not necessarily correction subsets of other related formulas, but satisfiable subsets do transfer between formulas.

\begin{proposition}\label{prop:ss}
if $\m{S} \subseteq \formula_1$ is a satisfiable subset of $\formula_1$, then $\m{S}\cap \formula_2$ is a satisfiable subset of $\formula_2$.\bart{Not so convincing to call this a proposition (rather than ``trivial observation''}
\end{proposition}
Hence, $\formula_2 \setminus \{\m{S}\cap \formula_2\}$ is a correction subset of $\formula_2$ and can be added to the sets-to-hit $\setstohit$. As before, we can first grow the satisfiable subset $\grow(\m{S}\cap \formula_2,\F_2)$ to obtain a larger satisfiable subset and hence a smaller MCS.
...
For instance, in the explanatory setting, we know that $\formulac$ is satisfiable with (partial) solution $\Iend$, and is a satisfiable subset for \emph{all} \omusinc calls to follow. 
