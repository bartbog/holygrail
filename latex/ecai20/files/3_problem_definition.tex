...
\bart{Don't have anything against it. 
I would call $C\land B\limplies A$ plus a visualisatoin thereof an explanation, but the pure mathematic object $(C,B)$ is not really an explanation yet. (not sure if we have to make this distinction in the text)
}
...
\bart{Rather,
Alternatively, this problem can be cast as the problem of finding a MUS of the formula $I_i\land S\land \lnot N_i$. (we need the negation for something to become UNSAT)}
