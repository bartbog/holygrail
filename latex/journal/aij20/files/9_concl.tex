...
\end{itemize}
These observations suggest that further research into the question \emph{what constitutes an understandable explanation for humans} is needed with respect to \emilio{using appropriate interpretability metrics \cite{hoffman2018metrics,rosenfeld2021better}}. Additional directions to produce easier-to-understand explanations would be optimizing the entire sequence instead of step by step, and learning the cost function based on user traces. 
\emilio{Finally, since the nested explanations for tested puzzles were so simple that no further refinement was needed. This might not be the case for more difficult problems. Thus, a possible avenue for future work is a generic approach to multi-level nested explanations using explanation sequence abstraction and refinement\cite{leuschel2005automatic, saribatur2019abstraction, mitchell2008expressive}.}
