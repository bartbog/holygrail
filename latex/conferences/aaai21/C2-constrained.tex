...
%     (sat?, $\kappa$) $\gets$ \texttt{SatSolver}($hs$)\;
  ...
  \caption{$\comus(\formula,f,p,\setstohit=\emptyset)$ }
  \label{alg:comus}
\end{algorithm}
...
$I \gets \emptyset$\;
$p \gets \{$exactly one of $\bar{I}_{end}$ in the hitting set$\}$\; %, none of $I_{end}$ can be in the hitting set$\}$\;
...
\Return{E}\;
\end{algorithm}
\tias{dit is eigenlijk het greedy algo, zou in sec 3 moeten komen... met lijn 12/13 dan door 'SingleStepExplain(C,f,I,Iend)'. Zoals algo 2; hieronder de 3 versies (sec 5,6,7):}
\begin{algorithm}[ht]
  \caption{$\call{bestStep1}({\cal C},f,I,I_{end},\setstohit)$}
  \label{alg:singleStepExplain1}
...
\end{algorithm}
\begin{algorithm}[ht]
  \caption{$\call{bestStep2}({\cal C},f,I,I_{end},\setstohit)$}
  \label{alg:singleStepExplain2}
$X_{best} \gets \{{\cal C} \land I \land \bar{I}_{end}\}$\;
\For{$l \in \{I_{end} \setminus I\}$}{
    $X \gets \omusinc{({\cal C} \land I \land \neg l, f, \setstohit)}$\;
    \If{$f(X)<f(X_{best})$}{
        $X_{best} \gets X$\;
    }
}
...
\end{algorithm}
\begin{algorithm}[ht]
  \caption{$\call{bestStep3}({\cal C},f,I,I_{end},\setstohit)$}
...
\Return{$\comus(\formulag,f,p,\setstohit)$}\;
\end{algorithm}
