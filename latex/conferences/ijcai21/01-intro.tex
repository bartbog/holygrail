...
\begin{compactitem}
 \item We develop algorithms that compute \textbf{\emilio{(cost-)}Optimal} Unsatisfiable Subsets (from now on called OUSs) based on the well-known hitting-set duality that is also used for generating cardinality-minimal MUSs \cite{ignatiev2015smallest}.
...
% \end{compactitem}
\todo{Add if explanations are used in practice or if they are only a recently introdyuced concept that still needs further development.}
\emilio{Ultimately computing an \mm{\call{OCUS}} also benefits a number of applications relying on finding inconsistencies as explanations to solve the problem by being able to formulate explanation-preferences as constraints and using the optimality criterion.}
