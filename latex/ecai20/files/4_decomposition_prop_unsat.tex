The goal is to search for a sequence of $I' /wedge C' \rightarrow A$ explanations, such that each explanation is as simple to grasp as possible and the sequence explains the steps to a full solution/interpretation. \textbf{TODO: adapt to formalism/language used in previous section}
...
\begin{algorithmic}
\Require{$C = $ all constraints}
\State{Seq $\Leftarrow$ empty sequence}
\State{$I \Leftarrow \{\}$}
\While{$I$ not a full interpretation}
  \State{$(I', C', A) \Leftarrow $smallest-explanation$(I, C)$}
  \State{append $(I', C', A)$ to Seq}
  \State{$I \Leftarrow I \cup A$}
\EndWhile
\end{algorithmic}
...
\Function{candidate-explanations}{I, C}
  ...
  \State{$J \Leftarrow$ optimal-propagate$(I \wedge C)$}
  \For{$a \in J \setminus I$} \Comment{Find a minimal expl. for each new fact}
    \State{$X \Leftarrow MUS(\neg a \wedge I \wedge reify(C))$}
    \State{$I' \Leftarrow I \cap X$}
    \State{$C' \Leftarrow C \cap X$}
    \State{$A \Leftarrow$ optimal-propagate$(I' \wedge C')$} \Comment{All implied facts}
    \State{add $(I', C', A)$ to Candidates}
  \EndFor
\EndFunction
\end{algorithmic}
...
\begin{algorithmic}
\Require{$I = $ partial interpretation, $C =$ all constraints}
\Function{smallest-explanations}{I, C}
  \State{Candidates $\Leftarrow$ empty stack}
  \For{$C' \subseteq C$ ordered by $g(C')$}
    \If{ $g(C') < min($Candidates$)$}
      \State{Break}
    \EndIf
    \State{cand $\Leftarrow$ candidate-explanations$(I, C')$}
    \State{add to Candidates all cand$_i$ with corresp. value $f($cand$_i)$}
  \EndFor
  \State{Return smallest scoring candidate of Candidates}
\EndFunction
\end{algorithmic}
