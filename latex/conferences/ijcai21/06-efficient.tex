...
While our OCUS algorithm in~\ref{alg:comus} is generic and can also be used to find OUSs, \emilio{i.e by removing predicate $p$. In algorithm \ref{alg:comus}, this boils down to replacing line 2 with $\cohs(\setstohit,f,True)$}. However, its constrainedness property is key to decreasing the complexity of explanation sequence generation.
...
\emilio{Indeed, if the computation cost of the \grow were not important, we could use a \maxsat call to obtain a subset-maximal subset of $\formula$. However, as it will become clear from the experiments, we expect the \grow costs to be too high to make the overall OCUS algorithm faster. }
