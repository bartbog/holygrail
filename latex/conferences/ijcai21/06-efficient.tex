...
\paragraph{example 1 (cont.)} Consider line 0 in table \ref{tab:explanation-steps-expanded}. During the \grow of the \maxsat solver \emph{Max-Actual-Unif} with polarities set to \Iend, it branches when multiple assignment to a literal are possible. By hinting with the polarities, we guide the solver and it assigns all values according to the end interpretation and neither $c_6$ nor $c_7$ is taken.
