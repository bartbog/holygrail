...
Similarly to the algorithm \ref{alg:main}, the \textit{nested-explanation} exploits the \textit{min-explanation} function to generate the candidate nested explanations. 
However, for a nested explanation sequence, the search space for the next easiest explanation-step is bounded by the cost-function and the explanations it can use which ultimately also serves for efficiency purposes. 

Given an explanation step $(E, S, N)$ and the new information $n_i \in N$ for which we want more explanations, algorithm \ref{alg:nested-explanation} first constructs a partial interpretation $I'$ formed by the explanations and the negated new fact $n_i$. 
Then, we gradually build the sequence by adding the newly found explanations $(E', S', N')$ as long as the interpretation is consistent and the explanation is easier than explanation step $(E, S, N)$, 
In the end, the algorithm inherently recognises the complex inference steps for which it builds nested explanation sequences.
