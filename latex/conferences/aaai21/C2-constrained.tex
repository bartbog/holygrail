Even when using an incremental OUS algorithm, at each step we still need to call \omusinc for every literal.

We now aim to instead use an OUS-like algorithm to directly find the best OUS across all literals in $I_{end} \setminus I$, where $I$ is the set of already explained literals, as in Algorithm~\ref{alg:bestStepOUS}. 
...
Correctness of the algorithm now follows from the fact that -- as before -- all sets added to \setstohit are correction subsets (and that every MUS must thus hit all sets in \setstohit) and \cref{prop:K2}, which guarantees correctness and completeness.
