...
\begin{table}[!h]
	...
	\begin{adjustbox}{max width=\columnwidth}
			\begin{tabular}{|r||c|c|c|c|}
				...
				\hline
				\rule{0pt}{2ex} 0 & - &  -&-  & \rule{0pt}{2ex} $\{ \{ c_5, c_6\} \}$\bart{$\emptyset$}  \\
				\hline
...
%<<<<<<< HEA
Line 0 in table \ref{tab:explanation-steps-expanded} is the initialization phase, i.e bootstrapping the \setstohit.
\bart{drop}
The next lines represent the successive hitting sets $\m{S}$ computed by the hitting set solver. The call to \grow exemplifies a call to \maxsat solver with hard clauses (or infinite weights) on the hitting set clauses and uniform weights on the remaining clauses of $\formula$.\bart{I don't think that this can at the moment be followed. I would say ``In this example, the \grow we used is the one called ''... in \cref{ref}.  } 

We further detail in section~\ref{sec:experiments} which \grow-configuration works best. Finally, note how at line 2, the hitting set computed is not $\{ c_5, c_6 \}$ of cost 2 due to the `\textit{exactlyone}'-constraint. \bart{Euh... and because there is a set to hiet containing only c4???? What is this?}
