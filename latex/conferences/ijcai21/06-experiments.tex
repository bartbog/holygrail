...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{figures_post_paper/cumulative_lits_grow.pdf}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.9\columnwidth]{figures_post_paper/cumulative_lits_derived_time.pdf}
...
To our surprise, we noticed that \emilio{both in the constrained and non-constrained OUS setting, incrementality had no noticeable effect. Even more so, the naive restart of the search towards an optimal (constrained) explanation provides improvements on the more difficult puzzles. One potential explanation is that when keeping the MIP solver warm, similar branch-and-bound based solver will start in a search space close to the previous solution . However there is no immediate guarantee that the new best explanation is closely related to the previous one.}
