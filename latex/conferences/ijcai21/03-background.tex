...
\begin{definition}\label{def:minimal-hs}
    ...
    \emilio{modified K to H for consistency with algorithm}
\end{definition}
...
%     
    A set  $\m{S} \subseteq \formula$ is an MCS of $ \formula$ iff  it is a \emph{minimal hitting set} of \muses{F}.
% 
%     \noindent
    A set  $\m{S} \subseteq \formula$ is a MUS of $ \formula$ iff  it is a \emph{minimal hitting set} of \mcses{F}.
\end{proposition}
