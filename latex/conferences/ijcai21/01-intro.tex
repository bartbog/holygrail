...
However, using multiple MUS calls per literal in each iterations quickly causes efficiency problems, with that explanation generation process taking several hours.
...
\begin{compactitem}
 \item We develop algorithms that compute \textbf{(cost-)Optimal} Unsatisfiable Subsets (from now on called OUSs) based on the well-known hitting-set duality that is also used for computing cardinality-minimal MUSs \cite{ignatiev2015smallest}.
...
% \tias{This paragraph not appropriate in an introduction because we don't actually do it. Move to discussion/future work or discuss on Teams}.
% \emilio{Moved to future work}
