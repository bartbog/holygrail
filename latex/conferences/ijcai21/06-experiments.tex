...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.86\columnwidth]{figures_post_paper/cumul_incr_avg_time_lits_derived.pdf}
...
When looking at the runtime to explain the entire sequence, best results are obtained with OUS + Lit. Incr. HS, that is, using an incremental hitting set solver \textit{for every individual literal to explain}. However, for the first literals, we can see that it takes much more computation time and that reducing number of OUS calls from $n$ to $1$ per explanation step improves runtime (OCUS), but making each of the $n$ calls incremental across the entire explanation sequence generation process leads to an even faster process overall (OUS+Lit Incr HS).
