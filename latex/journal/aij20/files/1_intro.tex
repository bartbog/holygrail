...
That is, for explaining a single step in more detail we work using reasoning by contradiction: starting from the negation of a consequence of the difficult step, we can explain how this leads to a contradiction and hence requires the consequence. In essence, we assume the negation of the derived fact, and can reuse the principle of explanation steps up to the inconsistency and hence contradiction. 
...
% Publication history
\paragraph{Publication history} This paper is an extension of previous papers presented at workshops and conferences \cite{claesuser,DBLP:conf/bnaic/ClaesBCGG19,ecai/BogaertsGCG20}. The current paper extends the previous papers with more detailed examples, additional experiments, as well as the formalization of what we call \emph{nested explanation sequences} (an idea that first appeared in a demo we submitted to IJCAI-PRICAI 2020 but that is first formalized in the current paper).  
