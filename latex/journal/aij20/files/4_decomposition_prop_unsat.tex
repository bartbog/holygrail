...
\begin{algorithm}
  ...
  \Input{A partial interpretation $I$ and a set of constraints $\allconstraints$}
  \todelete{$\mathit{best}\gets none$\;}
  \emilio{$N \gets$ propagate$(I \wedge \allconstraints)$\;}
  \emilio{$\mathit{best}\gets (I, \allconstraints, N)$\;}
  \For{$S \subseteq \allconstraints$ ordered ascending by $g(S)$}{ \label{alg:min:for}
    ...
    cand $\gets$ best explanation from candidate-explanations$(I, S)$\;
        \If{\todelete{$\mathit{best} = none$ {\bf or}} $f(\mathit{best}) > f(cand)$}{$ \mathit{best}\gets cand$\; \label{alg:min:gets}}
  }
   \Return{$\mathit{best}$}
