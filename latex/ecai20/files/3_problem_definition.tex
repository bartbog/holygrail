...
The overarching goal is to generate a sequence of small reasoning steps, each with an interpretable explanation. We first introduce the concept of an explanation of a reasoning step, after which we define a cost function for a reasoning step and the cost of a sequence of reasoning steps. 

\paragraph{Explanation of reasoning steps.}
We assume given a theory $\allconstraints$ and initial partial interpretation $I_0$.

\begin{definition}
We define the \textbf{maximally consistent partial interpretation} of a theory $\allconstraints$ and partial interpretation $I_0$ as the partial interpretation $I_n$ obtained by propagating all constraints constraints in $\allconstraints$, with $I_0 \wedge \allconstraints \entails I_n$
\end{definition}
...
\begin{definition}
A \textbf{sequence of incremental partial interpretations} of a theory $\allconstraints$ with initial partial interpretation $I_0$ is a sequence $\langle I_0, I_1, \ldots, I_n \rangle$ where $\forall i>0, I_{i-1} \geqp I_{i}$ and $I_n$ is maximally consistent, e.g. $I_0 \wedge \allconstraints \entails I_n$
\end{definition} 

The goal is to obtain a sequence of incremental partial interpretations $\langle I_0, I_1, \ldots, I_n \rangle$ from initial partial interpretation $I_0$ to the globally consistent partial interpretation $I_n$:
...
A \textbf{non-redundent explained sequence of interpretations} is ...? Sync with the eventual definition of above
\end{definition} 
...
We will assume the existence of a cost-function $f(E_i,S_i,N_i)$ that quantifies the interpretability of a single explanation $S_i \wedge E_i \rightarrow N_i$ of a reasoning step $\langle I_{i-1}, I_i \rangle$ with $N_i = I_i \setminus I_{i-1}$. This is typically specific to the family of problems considered (see Section~\ref{sec:cost} for the size-based cost we use in our logic grid puzzle experiments).
...
\begin{definition}
Given a theory $\allconstraints$ and initial partial interpretation $I_0$ such that $I_0 \wedge \allconstraints \entails I_n$, the \textbf{explanation-production problem} consist of finding a sequence of interpretations $\langle I_0, I_1, \ldots, I_n \rangle$ with corresponding subset minimal explanations $(E_i,S_i,N_i)$ where $N_i =  I_i \setminus I_{i-1}$ such that a predefined aggregate over the vector $f(E_i,S_i,N_i) \forall_i$ is minimised.
\end{definition} 
