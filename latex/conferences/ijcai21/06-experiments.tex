...
\item The \maxsat solver (\emph{Max}) is combined with different weighing schemes: uniform weights (\emph{unif}), cost-function weights $w_i$, i.e., equal to the weights in $f$ (\emph{pos}) or the inverse of these costs (\emph{inv}) defined as $max(w) + 1 - w_i$.% \bart{DEFINE INVERSE! What did you implement. } 
\end{itemize}
