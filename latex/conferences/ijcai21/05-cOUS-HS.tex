...
Instead of directly adding the complement of $\m{S}$ to \setstohit as done by \citet{ignatiev2015smallest}, our algorithm includes a call to \grow, which extends $\m{S}$ into a larger subset of $\formula$ that is still satisfiable (if possible). \bart{AGAIN: DOES IGNATIEV NOT HAVE A GROW? A reviewer also asked this. We anwsered the reviewer that he was right: ignatiev has a grow. 
...
\emilio{The result of algorithm \ref{alg:comus} represents a subset of the clauses of $\m{F}$.} \bart{This sentence is a bit lost here. I would remove it. I think it is quite clear. 
Also: ``represents'' should be ``is''.}
\begin{example}
\bart{Let me know if this example was updated. I will read it again tomorrow.}
	\emilio{ Consider the following CNF formula $\formula$ over variables $x_1, x_2, x_3$ with interpretation $\{x_1, \lnot x_2, x_3 \}$:
	...
	\bart{``we already know $I=\{...\}$ says that ''I know that I equals something``. What you want to say is: I is the interpretation so far. I know that everything in I istrue, right?}
	...
	Let $\setstohit$ be the collection of hitting sets. We can help the hitting set solver by bootstrapping it with $\setstohit  =\{ \{ c_6, c_7\} \}$, to ensure at least one of the two clauses is taken. We also add the `\textit{exactlyone}' constraint to the hitting set solver. Table \ref{tab:explanation-steps-expanded} sketches the steps required to find an explanation for \formula.}
	...
	\begin{adjustbox}{max width=\columnwidth}
		\emilio{
			\begin{tabular}{|r||c|c|c|c|}
				...
				\hline
				\rule{0pt}{2ex} 0 &$\{ \emptyset \}$ & True & $\{ \emptyset \}$    & $\{ \{ c_6, c_7 \}\}$\rule{0pt}{2ex}   \\
				\hline
				1 &\rule{0pt}{2ex} $\{ c_6 \}$ & True & $\{ c_6,  c_1, c_5, c_2, c_3\}$    & $\{ \{ c_6, c_7\}, \{c_7, c_4\}\}$\rule{0pt}{2ex}   \\
				\hline		
				\multirow{2}*{2} &\rule{0pt}{2ex}\multirow{2}*{ $\{ c_6, c_4 \}$}  & \multirow{2}*{True} & \multirow{2}*{$\{ c_6,  c_4, c_2, c_3\}$}   & $\{ \{ c_6, c_7\}, \{c_7, c_4\}$  \\  
				& &  &    & $\{c_7, c_1, c_5\}\}$\rule{0pt}{2ex} \\\hline
				\multirow{2}*{3}&\rule{0pt}{2ex} \multirow{2}*{$\{ c_6, c_4, c_5 \}$}  & \multirow{2}*{True} &  \multirow{2}*{$\{ c_6,c_1, c_4, c_5, c_7 \}$}  & $\{ \{ c_6, c_7\}, \{c_7, c_4\}$  \\  
				&&  &    & $\{c_7, c_1, c_5\}, \{c_2, c_3\}\}$\rule{0pt}{2ex} \\\hline
				4& \rule{0pt}{2ex} $\{ c_6, c_4,c_5, c_2 \}$ & False & \multicolumn{2}{|c|}{$\{ c_6, c_4,c_5, c_2 \}$ is an OCUS of $\formula$.} \\
				\hline
			\end{tabular}
		}
	\end{adjustbox}
...
\emilio{Line 0 in Table \ref{tab:explanation-steps-expanded} represents the initialization phase. The next lines represent the successive hitting sets $\m{S}$ computed by the hitting set solver. The call to \grow exemplifies a call to \maxsat solver with hard clauses (or infinite weights) on the hitting set clauses and uniform weights on the remaining clauses of $\formula$. We further detail in section~\ref{sec:experiments} which \grow-configuration works best. Finally, note how at line 2, the hitting set computed is not $\{ c_6, c_7 \}$ of cost due to the `\textit{exactlyone}'-constraint.}\tias{last sentence broken, what cost?}
