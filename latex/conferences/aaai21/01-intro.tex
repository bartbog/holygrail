...
However, the high number of MUS calls needed has as a result that even for simple problems for which the unique solution can be found in a fraction of a second, the explanation generation process takes several hours.
...
%\todo{Also say this?}
%Finally, we investigate how the ideas of \citet{davies} that were originally developed for \hitsetbased algorithms for solving MAX-SAT can be generalized to the case of constrained OUS generation. 
...
\begin{itemize}
  ...
  \item We experimentally validate our algorithms in the context of generation explanations of CSPs, thereby showing improvements both in terms of the time required to find explanation sequences and the quality thereof. %of the developed explanation.
\end{itemize}
...
In \cref{sec:davies}\tias{maybe subsection of 4}, we discuss how the ideas of \citet{davies} can be transferred to the constrained OUS setting. 
