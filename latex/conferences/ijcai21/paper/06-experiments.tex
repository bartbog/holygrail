...
\begin{description}
 ...
 \item[Q4] Does \textbf{re-use} of information across the different iterations improve efficiency?
\end{description}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.5\columnwidth]{figures_post_paper/heatmap_costs_mus_cous.pdf}
...
\begin{itemize}
\item \emph{Max} refers to growing with a MaxSAT solver and \emph{Greedy} to growing using a heuristic method implemented by repeated sat calls, while %\item 
\emph{no-grow} refers to skipping the \grow step.
\item  \emph{Full} refers to using the full unsatisfiable formula $\mathcal{F}$  while \emph{Actual} refers to using only the constraints that hold in the final interpretation (see Section~\ref{sec:ocusEx}). For instance, for the MaxSAT-based calls, \emph{Actual} means that only the previously derived facts and the original constraints are taken into account when computing optimality. 
\item The \maxsat solver (\emph{Max}) is combined with different weighing schemes: uniform weights (\emph{unif}), cost-function weights (\emph{pos}) (equal to the weights in $f$),  or the inverse of these costs (\emph{inv}) defined as $\max_j(w_j) + 1 - w_i$.% \bart{DEFINE INVERSE! What did you implement. } 
\end{itemize}
...
We can observe that not using a grow strategy performs badly, as do weighted MaxSAT grows with costs $w_i$ (-Pos). Greedy grow strategies improve on not using a grow strategy, but not substantially. The two approaches that work best use the domain-specific knowledge of doing a MaxSAT grow on $\formulac\cup I$, with the unweighted variant the only one that never times out.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.78\columnwidth]{figures_post_paper/new_cumul_grow_avg_time_lits_derived_new.pdf}
...
To have a better view on how incrementality also affects OUSb, we add incrementality to it in the following ways:
\begin{itemize}
...
% 
Figure \ref{fig:incrementality_constraindness} shows the results, where % of the different variants, including the MUS-based variant discussed above. 
In this figure, 
the configurations are compared in a similar fashion to Figure \ref{fig:grow_strategies}.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.78\columnwidth]{figures_post_paper/cumul_incr_avg_time_lits_derived_new.pdf}
...
When comparing the configurations, we see that the plain \emph{MUS}-based implementation is faster than the \emph{O(C)US} implementations, as it solves a simpler problem (with worse quality results as shown in \textbf{Q1}). %cannot keep up with , as is to be expected since a harder problem is tackled.
Replacing MUS by bounded OUS calls (orange line) leads to a much larger computation cost. The generic SS caching technique adds additional overhead. 
...
When looking at the runtime to explain the entire sequence, best results are obtained with OUSb + Lit. Incr. HS, that is, using an incremental hitting set solver \textit{for every individual literal to explain}. However, for the first literals, we can see that it takes much more computation time and that reducing the number of OUS calls from $n$ to $1$ per explanation step improves runtime (OCUS). However, making each of the $n$ calls incremental and bounded across the entire explanation sequence generation process leads to an even faster process overall (OUSb+Lit. Incr. HS).

%While slower than MUS, the runtime at most doubles, and, as investigated in \textbf{Q1} much better quality results are found.


%However, carefully taking the sequential nature of the explanation generation into account can reduce runtimes considerably: either by making the hitting set solver of each OUS call incremental across the entire sequence (OUS+Lit. Incr. HS), or by using a single \textbf{constrained} (OCUS) call at every step.
%While slower than MUS, the runtime at most doubles, and, as investigated in \textbf{Q1} much better quality results are found.
...
%In the case of \comus, using a single MIP solver and its built-in incrementality capabilities across all step does not lead to further improvements and even induces a bit of overhead, %. %, we see only minor runtime improvements at the end of the sequence where the more difficult steps are. Surprisingly, for
%while for \emph{OUS+Lit.~Incr.~HS} it dramatically reduced the runtime with a an average total runtime very close to \comus but slightly different behaviour in earlier steps.
%For the first few literals, \emph{OUS+Lit.~Incr.~HS} is almost on par with \emph{MUS}.
...
%\emilio{As a final experiment, we investigate what the typical runtime is to explain one individual explain step for the top 3 configurations: \emph{\comus}, \emph{\comus+Incr.~HS} and \emph{OUSb+Lit.~Incr.~HS}. For both \emph{\comus} configurations, we see that $50\%$ of the explanations take less 1 second to be generated. Within 5 seconds, approximately $87\%$ of the explanations have been generated. In contrast, for \emph{OUSb+Lit.~Incr.~HS} it takes less than 2 second to generate $87\%$ of the explanations, with one explanation step which takes more than 1000 seconds. This expensive explanation step explains the jump on Figure~\ref{fig:incrementality_constraindness} not only for \emph{OUSb+Lit.~Incr.~HS} but for all \emph{OUSb} configurations.}
...
%This is shown in table \ref{tab:explanation-time}, where we can see that the major part of the explanations takes less than 2 to 5 seconds to be generated. On the other hand, a handful of explanations require 20 to 100+ seconds to be generated and dominated the total runtime for explaining a sequence. %Indeed, puzzles often involve many 'easy' derivation steps that involve few facts and constraints, and a few 'hard' derivation steps. Whether difficulty for a person solving a puzzle and an algorithm finding TRICKY... not writing it.
