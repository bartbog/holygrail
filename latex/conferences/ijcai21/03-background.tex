...
\begin{definition}
  A \emph{Minimal Unsatisfiable Subset} (MUS) of an unsatisfiable formula \F is a set $\m{S} \subseteq \F$  that is unsatisfiable such that every strict subset of $\m{S} $ is satisfiable. 
%   
  %We write 
...
It is well-known that each  MCS of \formula is the complement of an MSS of \formula and vice versa. \tias{This needs a citation, even if there are citations lower, Reiter?}
