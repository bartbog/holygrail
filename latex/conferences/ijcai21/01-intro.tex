...
In order to generate these sequences, they rely heavily on calls for  \emph{\underline{M}inimal \underline{U}nsatisfiable \underline{S}ubsets} (MUS) \cite{marques2010minimal} of a derived program, exploiting a one-to-one correspondence between so-called \emph{non-redundant explanations} and MUSs.
...
\begin{compactitem}
 \item We develop algorithms that compute \textbf{\emilio{(cost-)}\underline{O}ptimal} \underline{U}nsatisfiable \underline{S}ubsets (from now on called OUSs) based on the well-known hitting-set duality that is also used for generating cardinality-minimal MUSs \cite{ignatiev2015smallest}.
