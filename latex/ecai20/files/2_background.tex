...
\tias{a theory is a set of sentences reads the above, so this is weird... instead, 'use them as part of a theory when solving'? For me there is only one theory, the CSP, but for you there can be multiple, a reader like me may not see that or be confused} \bart{Hmm... And taking the union of two CSPs to obtain a third one for solving the combination, would that also sound strange. In the context of a single solver call, there is usually only one CSP. But in general there can be more for me... I rewrote the above in an attempt to clarify... }
% 
