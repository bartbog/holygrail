...
For MIP calls, we used Gurobi 9.0, for SAT call  minisat22\bart{??? Is this MiniSat 2.2?} and for MaxSAT calls RC2 \bart{no version number?} as bundled with PySAT. In the MUS-based approach we used PySAT's deletion-based MUS extractor MUSX~\cite{marques2010minimal}.
...
\begin{description}
 \item[Q1] What is the effect of requiring optimality of the generated MUSs on the \textbf{quality} of the generated explanations and \added{the generated explanation sequence}?\bart{What does the sequence do here? We do not study it}
...
\item The \maxsat solver (\emph{Max}) is combined with different weighing schemes: uniform weights (\emph{unif}), original weights, i.e., equal to the weights in the OCUS call (\emph{pos}) or the inverse of the original costs (\emph{inv}) \bart{DEFINE INVERSE! What did you implement. } 
\end{itemize}
...
\added{As expected, the \maxsat-\grow on the actual interpretation \emph{Max-Actual} indeed performs best, though only \emph{Max-Actual-Unif} is able to finish all explanations. When repeating this experiment for \omus the same pattern was observed. \bart{AND also for incremental and non-incremental, right? Are you actually sure of that. We noticed this in the IJCAI paper but as far as I know, all your expiermental data from that time was messed up... } 
...
However, when using a single \textbf{constrained} (OCUS) call at every step we see a dramatic improvement in runtime. While slower than MUS, the runtime at most doubles, and, as investigated in \textbf{Q1} much better quality results are found. %. then multiple MUS calls,  ,  constrainedness, by switching to Algorithm \ref{alg:oneStepOCUS}, we effectively reduce the number of calls to the \omus solver.
...
\emilio{Potential interpretation: For explanations requiring more time to be generated, i.e. more than 10 seconds, 80\% of the time is spent in proving optimality by the hitting set solver. We also notice that they are also associated a higher average cost. Whereas explanations with lower costs spend only 20\% of their time in the hitting set solver and close to 80\% of the time in \grow.}
...
\emilio{From what I see the explanations with higher times have higher costs and most of the time is spent in opt, while the others the time is mostly spent in grow.\\
$t > 20$ Avg. Cost = 189... \\ 

topt = 81.63 \% sat= 0.23\% grow= 12.42\\
$t < 20$  Avg. Cost = 74... \\ 

topt = 17.38 \% sat= 2.19\% grow= 79.34}
\bart{In any case: that is not an explanation, merely an observation. I don't know whether this is relevant. 
average cost is 74, this means basically: these are the ones where a single bijectivity or transitivity does the job.... } 
