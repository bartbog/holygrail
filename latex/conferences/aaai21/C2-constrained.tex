...
We can see that for an OUS of $\formulac \cup I_{end} \cup \overline{\Iend}$ to be a valid explanation, it needs to satisfy a number of constraints. Let $I$ be the set of already explained literals, as in Algorithm~\ref{alg:bestStepOUS}. We can identify the following constraints: 1) the OUS should explain exactly one literal $-l$ in $\overline{\Iend}$; 2) the OUS may not explain a literal that is already explained, that is, no literal in $\overline{\Iend} \setminus \bar{I}$; and 3) the explanation can only use literals already explained, so no literals from $I_{end} \setminus I$.
