...
%\end{frame}
	...
	\begin{block}{Optimal \emph{Constrained} Unsatisfiable Subset}
		...
		\end{itemize}
%		An \emph{\color{vuborange}optimal} constrained explanation of $\m{F} := \m{C} \wedge \m{I} \wedge \lnot l$ corresponds to computing an Optimal (cost-minimal)Constrained Unsatisfiable Subset (\emph{\color{vuborange}OUS}) with respect to cost-function $f: 2^{\m{F}} \rightarrow N$.
	\end{block}
